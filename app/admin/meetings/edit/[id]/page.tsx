...
export default function EditMeetingPage() {
  ...
  const [oldHeroImage, setOldHeroImage] = useState("");
  ...
  async function fetchMeeting() {
    try {
      ...
      if (!res.ok) {
        setError(data?.message || "Không lấy được dữ liệu meeting.");
        return;
      }
      ...
      setAgendaFileUrl(meetingData?.agendaFileUrl || "");
      setReportFileUrl(meetingData?.reportFileUrl || "");
      setPhotosLink(meetingData?.photosLink || "");
      setOldHeroImage(meetingData?.heroImage || "");

      setStartDate(toLocalDateTimeValue(meetingData?.startDate));
      setEndDate(toLocalDateTimeValue(meetingData?.endDate));
    } catch (error) {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (imageFile) {
        ...
        heroImage = uploadData?.fileUrl || oldHeroImage;
      }
      ...
      const updateRes = await fetch(`${API_URL}/api/meetings/${id}`, {
        method: "PUT",
        headers: {
      ...
      if (!updateRes.ok) {
        setError(updateData?.message || "Cập nhật thất bại.");
        return;
      }
  ...
  if (pageLoading) {
    return (
      <div className="min-h-screen bg-[#f5f6f8] p-4 md:p-6">
        <div className="max-w-3xl mx-auto bg-white rounded-3xl shadow-sm border border-gray-200 p-8">
      ...
      <div className="max-w-3xl mx-auto bg-white rounded-3xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="px-6 md:px-8 py-6 border-b border-gray-100">
          ...
          <p className="text-gray-500 mt-2">
            Cập nhật nội dung meeting để đồng bộ sang frontend.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 md:p-8 space-y-5">
          ...
          {oldHeroImage && (
            <div>
              ...
              <img
                src={getFileUrl(oldHeroImage)}
                alt="meeting"
                className="w-56 h-36 object-cover rounded-2xl border border-gray-200 bg-gray-50"
              />
            </div>
          )}
